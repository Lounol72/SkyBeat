...
Ce tutoriel explique comment utiliser l'API Spotify afin de récuperer des playlists à partir d'une recherche utilisateur.
...
Le backend sert d'intermediaire entre l'application Angular et l'API Spotify afin de gérer l'authentification, le cache et les requêtes.
...
Créer egalement une application sur le portail developpeur Spotify :
...
Le fichier lançant le serveur backend est \texttt{backend/serveur.js}.
...
La gestion du token Client Credentials est aussi faîte dans \texttt{backend/serveur.js}.
...
Le service utilisé est \texttt{src/app/services/spotify.service.ts}.

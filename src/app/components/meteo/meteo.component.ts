...
export class MeteoComponent implements OnInit {
  ...
  locationName = signal<string>('Loading location...');
  ...
  private getUserLocation(): void {
    if (!navigator.geolocation) {
      ...
      this.locationName.set('Paris, France');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        this.locationName.set(`${lat.toFixed(2)}°N, ${lon.toFixed(2)}°E`);
      },
      (error) => {
    ...
    this.http.get<any>(url).subscribe({
      next: (response) => {
        if (response.current) {
          ...
          this.error.set('Invalid response from weather service');
          this.isLoading.set(false);
        }
      },
      error: (err) => {
